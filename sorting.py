...
You cannot use these built-in
functions anywhere in this file.

Every function in this file
takes a comparator `cmp` as input
...
def _merged(xs, ys, cmp=cmp_standard):
    ...
    the elements of both xs and ys.
    Runs in linear time.
...
def quick_sorted(xs, cmp=cmp_standard):
    ...
    if len(xs) <= 1:
        ...
        mid = len(xs) // 2
        pivot = xs[mid]
        xs_ls = [x for x in xs if cmp(x, pivot) == -1]
        xs_gt = [x for x in xs if cmp(x, pivot) == 1]
        xs_eq = [x for x in xs if cmp(x, pivot) == 0]
        xs_ls = quick_sorted(xs_ls, cmp=cmp)
        xs_gt = quick_sorted(xs_gt, cmp=cmp)
        return xs_ls + xs_eq + xs_gt


def quick_sort(xs, cmp=cmp_standard):
    '''
    EXTRA CREDIT:
    The main advantage of quick_sort is
    that it can be implemented "in-place".
    ...
    (We say quick sort has a lower
    "constant factor" in its runtime.)
    ...
    def partition(a, lo, hi):
        ...
        for j in range(lo, hi):
            if cmp(pivot, a[j]) == 1 or cmp(pivot, a[j]) == 0:
                i += 1
                a[i], a[j] = a[j], a[i]
